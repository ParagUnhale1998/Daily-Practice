...
export class OwnerLoginComponent {
  ...
  onSubmit() {
    if (this.loginForm.valid) {
      ...
      this.toastr.showError('Please Enter Something || Relogin', 'Failed Login');
    }
  }
...
confirmPhoneNo(){
  ...
  this.ownerFoundData = this.ownersAll.find((owner:any) => owner.phoneNO === phoneNo);
  ...
  if (this.ownerFoundData) {
    ...
    this.ownerService.updateOwnerPassword(ownerId, newPassword).subscribe(
      (response) => {
        ...
        this.toastr.showSuccess('Password updated successfully', 'successfull');
